...
const LoginForm = () => {
  ...
  const [, setUser] = useRecoilState(userInfo); // to update the state of atom 
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (response.data) { // Check if user exists
        if (response.data.password === password) {
          setUser({ isLoggedIn: true, data: response.data });
          if (response.data.accountType === 'Tenant') {
      ...
      <div className="w-full max-w-md mx-auto bg-card border border-border rounded-lg shadow-lg p-8">
        ...
        <form onSubmit={handleLogin} >
          ...
          <div className="flex items-center mb-6">
            ...
            />
            <label htmlFor="remember" className="ml-3 text-sm text-foreground cursor-pointer font-inter font-[14px]">
              Remember Me
            </label>
          </div>
        ...
        <div className="text-center">
          ...
          <Link to="/signup" className="text-sm text-primary hover:text-primary/80 font-medium transition-colors">
            Register Now
          </Link>
        </div>
      </div>
